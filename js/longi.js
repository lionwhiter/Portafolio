 ...
 {
	//tamaño de Pantalla para la realización de calculo 
	//de tamaño de figuras 
	const letchFigure = ()=> {
		let letchScree = document.defaultView.screen,
		letch = [
			{
		...
		switch (letchScree.width) {
			case 360:
			case 384:
			case 412:
				letch = setFigure(letch,0.7);		
				break;
			case 810:
				letch = setFigure(letch,0.4);		
				break;

			default:
				console.log(letchScree.width)
				break;
		}
		return (letch);
	};

	const setFigure = (letch, valor) =>{
		console.log("elementos Internos de tamaño");
		letch.forEach(element => {
			element.scaleX -= valor;
	...
	const TiltObj = function(el, options) {
		...
		this.DOM = {};
		this.DOM.img = this.el.querySelector('.content--main');
	...
	DOM.contentElems = Array.from(document.querySelectorAll('.content--main'));
	...
	const init = function() {
		imagesLoaded(document.body, () => {
			...
			Array.from(document.querySelectorAll('.content--main')).forEach(el => new TiltObj(el));
